...
class Network(metaclass=ABCMeta):
    ...
    def plot_degree_distribution(self, net_type="Network", xmax=1500, plot_now=True):
        """
        Compute degree distribution data and optionally plot.
        
        Parameters:
        -----------
        net_type : str
            Name of the network type for labeling
        xmax : int
            Maximum x-axis value for plotting
        plot_now : bool
            If True, plot and save immediately. If False, return data dict.
        
        Returns:
        --------
        dict or None
            If plot_now=False, returns {'degrees': array, 'ccdf': array, 'name': str}
            If plot_now=True, returns None and saves plot to file
        """
        if self.adjacency is None:
            raise ValueError("Adjacency not built. Call generate_network() first.")
        ...
        if plot_now:
            ...
            plt.xlabel("Degree (k)")
            plt.ylabel("P(X > k)")
            plt.title(f"Complementary Cumulative Distribution of {net_type}")
            plt.xlim(right=xmax)
            ...
            # Return data for later plotting
            return {
                'degrees': deg,
                'ccdf': ccdf,
                'freq': freq,
    ...
    @staticmethod
    def plot_combined_distributions(networks_data, output_file="src/output/combined_degree_dist.png", xmax=2000,
                                    title="Complementary Cumulative Degree Distributions"):
        """
        Plot multiple network degree distributions on the same figure.
        
        Parameters:
        -----------
        networks_data : list of dict
            Each dict should have 'degrees', 'ccdf', and 'name' keys
        output_file : str
        ...
        for data in networks_data:
            plt.plot(data['degrees'], data['ccdf'], 
                    label=data['name'])
