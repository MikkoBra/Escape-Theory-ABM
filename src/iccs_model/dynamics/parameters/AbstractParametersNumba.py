...
class Parameters(ABC):
    ...
    def get_B_params(self, internal_strategy, model, t):
        neighbor_ids = self.burdensomeness.neighbors

        if len(neighbor_ids) == 0 or t < 0:
            neighbor_As = np.zeros(len(neighbor_ids), dtype=np.float32)
        else:
            neighbor_As = model.data["aversive_internal_state"][t, neighbor_ids]
